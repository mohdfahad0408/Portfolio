...
while True:
    a1,a=int(input("Enter the Admin Password Of Railway Reservation.: ")),2
    while a>1:
        a=random()*10000
        a,a=a//1,int(a)
        print("\n\n==============\ncapture=",a,"\n==============")
        o=int(input("enter capture"))
        if o==a:
            break
        else:
    ...
    if a1==123:
       ...
       try:
                ...
                crs=mydb.cursor()
                qry01="CREATE DATABASE IF NOT EXISTS railway_reservation_system"
                crs.execute(qry01)
                qry02="USE railway_reservation_system"
                crs.execute(qry02)
                ...
                qry04="USE railway_reservation_system"
                crs.execute(qry04) 
                qryo2="CREATE TABLE IF NOT EXISTS avail_Train(Srno int(10),Train_No int(15) primary key,Train_Name varchar(250),Train_from varchar(200),Train_to varchar(300))"
                crs.execute(qryo2)
                ...
                print("")  
                print(r"    _____         ___   _____   ____                ____   ____   _____                 ____           _____")
                print(r"      |   |\   | |   \    |    |    | |\   |       |    | |    |    |    |     |    |  |    | \     / |     |")
                print(r"      |   | \  | |    |   |    |____| | \  |       |____| |____|    |    |     |    |  |____|  \___/  |_____")
                print(r"      |   |  \ | |    |   |    |    | |  \ |       |\     |    |    |    |     | /\ |  |    |    |          |")
                print(r"    __|__ |   \| |___/  __|__  |    | |   \|       | \    |    |  __|__  |____ |/  \|  |    |    |    |_____|")
                print(r"   __________________________________________     ____________________________________________________________")
                                ...
                                if x==1:
                                        ticket_booking()
                                          ...
                                          print('''
                                                   ...
                                                   DEVELOPED BY MOHD FAHAD XII-a 2024-25  
                                                ''')
                ...
                def Available_ALL_Trains():
                        import mysql.connector as msc
                        mydb=msc.connect(host='localhost',user='root',passwd='123456',database='railway_reservation_system')
                        crs=mydb.cursor()
                        mydb.autocommit=True
                        crs.execute("railway_reservation_system") 
                        crs.execute("LOAD DATA LOCAL INFILE 'C:/Users/Jyoti/Desktop/Yash/Train.csv' into table avail_train fields terminated by ',' lines terminated by '\r\n' ;")         
                        ...
                        df = pd.DataFrame(data1, columns=["Srno","Train_No","Train_Name","Train_from","Train_to","Fare",])                                                
                        print(df)
                        mydb.commit()
                        crs.close()
                        return("")
                ...
                def btw_Stations():
                        ...
                        crs=mydb.cursor()
                        mydb.autocommit=True
                        crs.execute("use railway_reservation_system") 
